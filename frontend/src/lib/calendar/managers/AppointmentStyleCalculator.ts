...
export class AppointmentStyleCalculator implements IStyleCalculator{
  ...
  getAppointmentStyle(
    ...
    const width = this.calculateAppointmentWidth(appointment, allAppointments);
    const left = this.calculateLeftPosition(appointment);
  ...
  private calculatePositionAndHeight(appointment: PositionedAppointment) {
    const topPosition = (appointment.start.getHours() * 60 + appointment.start.getMinutes()) / 2;
  ...
  private calculateAppointmentWidth(
    ...
    const overlapping = this.overlapCalculator.findOverlappingAppointments(appointment, allAppointments);
  ...
  private calculateOverlappingWidth(overlapping: PositionedAppointment[]): number {
    return overlapping.reduce((total, app) => total + (100 / app.totalColumns), 0);
  }
